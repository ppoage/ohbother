...
class BuildGoBindings(Command):
    ...
    def run(self):
        ...
        with open(init_path, "w") as f:
            ...
            f.write(
                "from .ohbother import *  # Import all symbols from the Go bindings\n"
            )
        ...
        if sys.platform.startswith("win"):
            ...
            if os.path.exists(python3_path):
                ...
                print(
                    f"WARNING: python3.exe not found in {python_dir}, using {python_path}"
                )

        print(
            f"Using Python interpreter: {python_path} (from {'environment' if 'PYTHON_VM_PATH' in os.environ else 'sys.executable'})"
        )
        print(
            f"Platform: {sys.platform}, GOARCH: {env.get('GOARCH')}, GOOS: {env.get('GOOS')}"
        )
...
class CustomBdistWheel(bdist_wheel):
    ...
    def get_tag(self):
        ...
        # Debug output
        print(
            f"Original wheel tags: python_tag={python_tag}, abi_tag={abi_tag}, plat_tag={plat_tag}"
        )
    ...
    def run(self):
        ...
        if os.path.exists(ohbother_dir) and os.path.isdir(ohbother_dir):
            ...
            print(
                f"ERROR: Source directory {ohbother_dir} does not exist or is not a directory!"
            )
                ...
                for filename in os.listdir(nested_dir):
                    if filename.endswith(".py"):
                        ...
                        content = content.replace(
                            "from . import _ohbother", "from ohbother import _ohbother"
                        )
                        content = content.replace(
                            "from . import go", "from ohbother import go"
                        )
